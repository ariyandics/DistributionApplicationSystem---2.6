Public Class FrmUnPaket
    ...
    Private Sub FrmUnPaket_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub BtnFindNo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFindNo.Click
        ...
        txtnoMutasi.Text = FrmFind.cari("NoUnPaket")
    ...
    Private Sub loaddata()
        ...
        If flagedit = True Or flagadd = True Then
            strsql = "exec sptblProdukUnPaket 'getTMP',0,0,0," & NoMutasi & ",'x'"
        Else
            strsql = "exec sptblProdukUnPaket 'GetDetail',0,0,0," & NoMutasi & ",'x'"
        End If
    ...
    Private Sub cektable()
        ...
        If RsConfig.EOF Then
            sql = "select top 0 * into TblMutasiPaketTMP from TblUnPaketDetail "
            Conn.Execute(sql)
        End If
    ...
    Private Sub hitungTMP()
        If flagadd = True Or flagedit = True Then
            sql = "exec sptblProdukUnPaket 'HitungTMP',0,0,0," & NoMutasi & ",'" & StrNamaUser & "'"
        Else
            sql = "exec sptblProdukUnPaket 'GetHeader',0,0,0," & NoMutasi & ",'" & StrNamaUser & "'"
        End If
    ...
    Private Sub isitmp()
        sql = "exec sptblProdukUnPaket 'IsiTmp'," & idpaket & ",0," & qtyMutasi & "," & NoMutasi & ",'" & StrNamaUser & "'"
        Conn.Execute(sql)
    End Sub


    Private Sub hapustmp()
        sql = "exec sptblProdukUnPaket 'HapusTmp',0,0,0," & NoMutasi & ",'" & StrNamaUser & "'"
        Conn.Execute(sql)
    End Sub
    ...
    End Sub
    Private Sub simpan()
        Dim result2 As DialogResult = MessageBox.Show("Simpan Data Un-Paket ??", _
              "Question !!", _
              MessageBoxButtons.YesNo, _
              MessageBoxIcon.Question)
        If result2 = DialogResult.Yes Then
            sql = "exec sptblProdukUnPaket 'StockPkt',0," & idProduk & "," & qtyMutasi & "," & NoMutasi & ",'" & StrNamaUser & "'"
            RsConfig = Conn.Execute(sql)


            If Not RsConfig.EOF Then
                MsgBox("Stock hanya tersedia " & RsConfig("qty").Value, vbOKOnly + vbCritical, "Gagal Simpan")
                txtqty.Focus()
                txtqty.SelectAll()
                Exit Sub
            Else
                ...
                If flagadd = True Then
                    sql = "exec sptblProdukUnPaket 'add'," & idpaket & "," & idProduk & "," & qtyMutasi & "," & NoMutasi & ",'" & StrNamaUser & "'"
                    Conn.Execute(sql)
                    Call pesan(3)
                    Call kunci()
                    Call bersih()
                ...
                ElseIf flagedit = True Then
                    sql = "exec sptblProdukUnPaket 'Edit'," & idpaket & "," & idProduk & "," & qtyMutasi & "," & NoMutasi & ",'" & StrNamaUser & "'"
                    Conn.Execute(sql)
                    Call pesan(3)
                    Call kunci()
                    flagedit = False
    ...
    Private Sub BtnFindProduk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFindProduk.Click
        ...
        idpaket = 0
        sql = "exec sptblProdukUnPaket 'getid',0," & idProduk & ",0,0,'x'"
        RsConfig = Conn.Execute(sql)
        If Not RsConfig.EOF Then
            idpaket = RsConfig("idpaket").Value
        End If
    ...
    Private Sub approve()
        ...
        If result2 = DialogResult.Yes Then
            ...
            If Not RsConfig.EOF Then
                MsgBox("Stock hanya ada " & RsConfig("qty").Value, vbOKOnly + vbCritical, "Gagal Simpan")
                txtqty.Focus()
                txtqty.SelectAll()
                Exit Sub
            Else
                ...
                End If
                Call kunci()
                sql = "exec sptblProdukUnPaket 'Approve',0,0,0," & NoMutasi & ",'" & StrNamaUser & "'"
                Conn.Execute(sql)
                LblTglMutasi.Visible = True
    ...
    Private Sub txtnoMutasi_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtnoMutasi.TextChanged
